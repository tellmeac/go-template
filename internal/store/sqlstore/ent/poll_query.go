...
func (pq *PollQuery) QueryPlayers() *PollPlayerQuery {
	query := &PollPlayerQuery{config: pq.config}
...
func (pq *PollQuery) FirstID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (pq *PollQuery) FirstIDX(ctx context.Context) ulid.ULID {
...
func (pq *PollQuery) OnlyID(ctx context.Context) (id ulid.ULID, err error) {
	var ids []ulid.ULID
...
func (pq *PollQuery) OnlyIDX(ctx context.Context) ulid.ULID {
...
func (pq *PollQuery) IDs(ctx context.Context) ([]ulid.ULID, error) {
	var ids []ulid.ULID
...
func (pq *PollQuery) IDsX(ctx context.Context) []ulid.ULID {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *PollQuery) Clone() *PollQuery {
	...
	return &PollQuery{
		...
		withPlayers: pq.withPlayers.Clone(),
...
func (pq *PollQuery) WithPlayers(opts ...func(*PollPlayerQuery)) *PollQuery {
	query := &PollPlayerQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPlayers = query
	return pq
}
...
func (pq *PollQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Poll, error) {
	...
	if query := pq.withPlayers; query != nil {
		if err := pq.loadPlayers(ctx, query, nodes,
			func(n *Poll) { n.Edges.Players = []*PollPlayer{} },
			func(n *Poll, e *PollPlayer) { n.Edges.Players = append(n.Edges.Players, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PollQuery) loadPlayers(ctx context.Context, query *PollPlayerQuery, nodes []*Poll, init func(*Poll), assign func(*Poll, *PollPlayer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[ulid.ULID]*Poll)
	for i := range nodes {
	...
	query.Where(predicate.PollPlayer(func(s *sql.Selector) {
		s.Where(sql.InValues(poll.PlayersColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "PollID" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
